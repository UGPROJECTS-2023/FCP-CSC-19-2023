...
<?php
 ...
 include("config.php");
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_delete = $conn->prepare('DELETE FROM orderdetails WHERE order_id =:order_id');
...
<?php
	...
	{
		$stmt_delete = $conn->prepare('update orderdetails set order_status="Ordered" WHERE order_status="Pending" and user_id =:user_id');
...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
			...
			<div class="alert alert-default" style="color:white;background-color:#CF7B4F">
...
include("config.php");
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
		 ...
		 include("config.php");
		  $stmt_edit = $conn->prepare("select sum(order_total) as totalx from orderdetails where user_id=:user_id and order_status='Pending'");
		$stmt_edit->execute(array(':user_id'=>$user_id));
		$edit_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
		extract($edit_row);
		...
		echo "</td>";
		echo "<td>";
		echo "<a  href='?update_id=".$user_id."' >
		<img src='pay.jpg' width='300' height='150'>
		</a>";
		echo "</td>";
		// echo "<td>";
		// echo "<a class='btn btn-block btn-success' href='card.php'><span class='glyphicon glyphicon-shopping-cart'></span> Order Now</a>";
		// echo "</td>";
		...
		echo '<div class="alert alert-default" style="background-color:#033c73;">
                       ...
                       &copy 2023 Best Online Shoping| Design by : ABDULGANIYU MUSTAPHA ABAYOMI
