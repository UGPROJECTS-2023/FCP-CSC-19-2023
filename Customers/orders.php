...
<body>
	<h2 style="text-align: center;color: red;margin-left: 195px;">Your order has been place. You will receive it in 24 hours</h2> 
    <div id="wrapper">
...
include("config.php");
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
		 ...
		 include("config.php");
		  $stmt_edit = $conn->prepare("select sum(order_total) as totalx from orderdetails where user_id=:user_id and order_status='Ordered'");
		$stmt_edit->execute(array(':user_id'=>$user_id));
		$edit_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
		extract($edit_row);
